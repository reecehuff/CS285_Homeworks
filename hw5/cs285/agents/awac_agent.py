...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        qvals = critic.q_net(obs)
        q_value = torch.gather(qvals, 1, action.unsqueeze(1)).squeeze(1)
        return q_value
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        dist = None # We don't need to use Boltzmann distribution here
        ...
        if self.agent_params['discrete']:
            ...
            for _ in range(n_actions):
                pass
        v_pi = torch.stack(vals, dim=1).mean(dim=1)
